...
class TestMapVectors(unittest.TestCase):
    ...
    def test_vertice_angles_by_poly_size(self):
        self.assertEquals(vertice_angles_by_poly_size(4), [0, 0.5 * math.pi, 1 * math.pi, 1.5 * math.pi - (2 * math.pi)])
    ...
    def test_find_index_of_closest_vector2vertice_in_new_poly(self):
        self.assertEquals(find_index_of_closest_vector2vertice_in_new_poly(math.pi/2, 4), 1)
        self.assertEquals(find_index_of_closest_vector2vertice_in_new_poly(math.pi/4, 4), 0)
        self.assertEquals(find_index_of_closest_vector2vertice_in_new_poly(math.pi * 3 / 4, 4), 1)
